...
/*   Updated: 2023/04/08 15:49:44 by gateixei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	str[i][j] = 'F';
	if (str[i][j + 1] && (str[i][j + 1] == 'C'\
	|| str[i][j + 1] == '0' || str[i][j + 1] == 'E'))
		check_all_possibilities(str, i, (j + 1));
	if (str[i][j - 1] && (str[i][j - 1] == 'C'\
	|| str[i][j - 1] == '0' || str[i][j - 1] == 'E'))
		check_all_possibilities(str, i, (j - 1));
	if (str[i + 1] && (str[i + 1][j] == 'C'\
	|| str[i + 1][j] == '0' || str[i + 1][j] == 'E'))
